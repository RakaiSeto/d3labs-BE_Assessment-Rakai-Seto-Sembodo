...
// Example usage:
const targetUnixTimestamp = Math.floor(Date.now() / 1000); // Example timestamp (replace with yours)

async function findBlockByTimestamp(provider, targetTimestamp) {
    let startBlock = 0; // Or a more recent block if you have an estimate
    ...
    while (startBlock <= endBlock) {
        ...
        if (!block) {
            // Handle cases where the block doesn't exist (e.g., during chain reorgs)
            console.error(`Block ${midBlock} not found.`);
            // You might want to adjust the search range or break
            break;
        }

        const blockTimestamp = block.timestamp;

        // Define an acceptable tolerance for the timestamp difference (e.g., a few minutes)
...
async function getHolder(provider, contractAddress, targetBlockNumber) {
    const contract = new ethers.Contract(contractAddress, contractABI, provider);
    const holder = contract.filters.Transfer();

    let fromBlock = 12286690; // Start from the first block when the contract was deployed
...
const main = async () => {
    ...
    if (block) {
        ...
        await getHolder(provider, contractAddress, block.number);
    } else {
